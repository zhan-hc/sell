...
export default {
  ...
  data () {
    return {
      slType: this.selectType, // 父组件传的值
      OnContent: this.onlyContent
    }
  },
  methods: {
    select (type) {
    ...
    toggleContent () {
      this.OnContent = !this.OnContent
      this.$emit('content_toggle', this.OnContent)
    }
  },
  computed: {
    positives () {
      return this.ratings.filter((rating) => { // 过滤函数将符合条件的数组成新的数组
